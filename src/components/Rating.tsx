...
export function Rating({ wrestler, compact = false }: RatingProps) {
  ...
  return (
    <div
      ...
      aria-label={
        average
          ? t('profile.masks', { count: average })
          : t('rating.waiting', { count: minimumVotes })
      }
    >
      <div className="rating__masks">
        {[1, 2, 3, 4, 5].map((score) => (
      ...
      <span>
        {voteCount < minimumVotes
          ? t('rating.more', { count: minimumVotes - voteCount })
          : t('rating.fanVotes', { count: voteCount })}
      </span>
    </div>
  );
}
