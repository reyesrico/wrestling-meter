...
export function SiteHeader({ signedIn, onSearch, onSignIn, onSignOut }: SiteHeaderProps) {
  ...
  const activeCompany =
    location.pathname === '/roster' ? new URLSearchParams(location.search).get('company') : null;
  const rosterActive =
    location.pathname === '/roster' && !companies.some((company) => company.id === activeCompany);
  ...
  return (
    <header className="site-header">
      ...
      >
        ...
        <Link className={rosterActive ? 'active' : ''} to="/roster">
          {t('header.roster')}
        </Link>
        {companies.map((company) => (
          <Link
            key={company.id}
            className={activeCompany === company.id ? 'active' : ''}
            to={`/roster?company=${company.id}`}
          >
            {company.shortName}
          </Link>
        ))}
