...
export function RosterPage() {
  ...
  const active: CompanyId | 'all' = companies.some((company) => company.id === requested)
    ? requested!
    : 'all';
  const sort = rosterSorts.includes(requestedSort as RosterSortValue)
    ? requestedSort!
    : 'meter-desc';
  ...
  const visible = (
    ...
    if (firstRating === null)
      return secondRating === null ? collator.compare(first.name, second.name) : 1;
    if (secondRating === null) return -1;
    return (
      (firstRating - secondRating) * (sort === 'meter-asc' ? 1 : -1) ||
  ...
  const changeCompany = (companyId: CompanyId | 'all') =>
    updateParam('company', companyId === 'all' ? null : companyId);
  const changeSort = (nextSort: RosterSortValue) =>
    updateParam('sort', nextSort === 'meter-desc' ? null : nextSort);

  return (
    <main className="page shell">
      <div className="page-heading">
        <span className="eyebrow">{t('roster.promotions')}</span>
